...
chromadb 允许使用 kubernetes >= 28.1.0
```
...
当前 `back/rag/image_parser.py` 还不能直接在新电脑使用，因为它包含原电脑的旧硬编码：

```python
MODEL_PATH = Path(r"D:\AI-models\PaddleOCR-VL-1.6")
device="gpu:0"
engine="paddle"
```
...
```text
通过 PaddlePaddle/PaddleOCR-VL-1.6 模型 ID 动态解析缓存路径
device="cpu"
删除已经失效的 engine="paddle"
```

这部分将在继续开发 DOCX 图片解析时修改。
...
`data/chroma_db` 是可重新生成的本地向量数据库。

迁移到另一台电脑后，建议使用知识库 DOCX 重新执行：
