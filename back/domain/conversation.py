...
ScopeType = Literal[
    "in_scope",      # 属于客服业务范围，需要继续识别意图或检索知识库。
    "chitchat",      # 简单问候、感谢或告别，不需要检索知识库。
    "out_of_scope",  # 明确不属于业务范围，例如天气、新闻等问题。
    "uncertain",     # 信息太少，无法判断是否属于客服业务范围。
]

# 业务意图类型：约束 intent 字段，用来表示用户具体在咨询哪类业务问题。
IntentType = Literal[
    "company_info",         # 企业介绍、产品用途、业务范围、营业时间、联系方式等基本资料。
    ...
    "other_business",       # 属于企业业务，但不属于以上已定义类别的问题。
