...
这个模块暂时不调用 PaddleOCR-VL，也不写入 OpenSearch。
...
class ExtractedImageRecord:
    ...
    def to_index_metadata(self) -> dict:
        """
        转换成检索索引可以接收的扁平 metadata。

        检索 metadata 不直接保存 Path、嵌套字典
...
def main():
    ...
    只提取图片并打印位置，不调用 OCR，也不写 OpenSearch。
    """
