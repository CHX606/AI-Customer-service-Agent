...
from back.knowledge.images.semantics import fingerprint_image
...
def _build_page_content(
    ...
    previous_text = str(
        image_result.get("previous_text", "")
    ).strip()
    next_text = str(
        image_result.get("next_text", "")
    ).strip()
    ...
    if previous_text:
        lines.extend(["图片上文：", previous_text])
    if next_text:
        lines.extend(["图片下文：", next_text])
    lines.extend(["图片识别信息（辅助证据）：", ocr_text])
    return "\n".join(lines)
...
def load_image_documents(
    ...
    for image_entry in image_entries:
        ...
        full_image_result = image_result.get("full_image_result", {})
        image_sha256 = (
            str(full_image_result.get("image_sha256", ""))
            if isinstance(full_image_result, dict)
            else ""
        )
        image_dhash = ""
        image_detail_dhash = ""
        image_width = 0
        image_height = 0
        full_image_file = Path(full_image_path)
        if full_image_path and full_image_file.exists():
            fingerprint = fingerprint_image(full_image_file)
        ...
        documents.append(
            Document(
                ...
                metadata={
                    ...
                    "image_detail_dhash": image_detail_dhash,
