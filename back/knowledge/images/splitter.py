...
- 第 7～11 张是故障定位参考图，OCR 优先识别完整原图；
  这里只预先生成整图识别不足时才会使用的兜底区域。

这里采用“整图优先 + 诊断区域兜底”的双层结构：
...
诊断区域
    仅在整图 OCR 为空、过短或失败时使用，放大真正影响
    故障判断的界面部分。

本模块只负责分类、裁剪和保存清单，不调用 OCR 模型，
也不写入 OpenSearch。运行和检查裁剪结果后，再接入
image_parser.py。
"""
...
from back.knowledge.ingestion.docx.image_extractor import (
    extract_docx_images,
)
...
class ReferenceImageTemplate:
    ...
    title: str
    diagnostic_goal: str
    required_ocr_term_groups: tuple[tuple[str, ...], ...]
...
REFERENCE_IMAGE_TEMPLATES = {
    7: ReferenceImageTemplate(
        ...
        required_ocr_term_groups=(("订阅",), ("节点",)),
        regions=(
            RegionTemplate(
    ...
    8: ReferenceImageTemplate(
        ...
        required_ocr_term_groups=(("订阅",), ("地址", "链接")),
        regions=(
            RegionTemplate(
    ...
    10: ReferenceImageTemplate(
        ...
        required_ocr_term_groups=(("127.0.0.1",), ("10808",)),
        regions=(
            RegionTemplate(
    ...
    11: ReferenceImageTemplate(
        ...
        required_ocr_term_groups=(("退出登录", "退出登陆"),),
        regions=(
            RegionTemplate(
        ...
        reference_images.append(
            {
                ...
                "required_ocr_term_groups": [
                    list(group)
                    for group in template.required_ocr_term_groups
                ],
